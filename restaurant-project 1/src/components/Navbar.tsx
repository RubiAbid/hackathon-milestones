import React from "react";
import Menu from "./Menu";
import Link from "next/link";
import CartIcon from "./Carticon"; // Fixed spelling (CartIcon)
import Image from "next/image";

const Navbar = () => {
  const user = false; // Simulated user login status

  return (
    <div className="h-16 text-red-500 p-4 flex items-center justify-between border-b-2 border-b-red-500 uppercase md:h-24 lg:px-20 xl:px-40">
      {/* LEFT LINKS */}
      <div className="hidden md:flex gap-4 flex-1">
        <Link href="/" className="hover:text-gray-700">Homepage</Link>
        <Link href="/menu" className="hover:text-gray-700">Menu</Link>
        <Link href="/contact" className="hover:text-gray-700">Contact</Link> {/* Contact Link */}
      </div>

      {/* LOGO */}
      <div className="text-xl md:font-bold flex-1 md:text-center">
        <Link href="/" className="hover:text-gray-700">Savoré Feast</Link>
      </div>

      {/* MOBILE MENU */}
      <div className="md:hidden">
        <Menu />
      </div>

      {/* RIGHT LINKS */}
      <div className="hidden md:flex gap-4 items-center justify-end flex-1">
        <div className="md:absolute top-3 right-2 lg:static flex items-center gap-2 cursor-pointer bg-orange-300 px-1 rounded-md">
          <Image src="/phone.png" alt="Phone icon" width={20} height={20} />
          <span>123 456 78</span>
        </div>
        {!user ? (
          <Link href="/login" className="hover:text-gray-700">Login</Link>
        ) : (
          <Link href="/orders" className="hover:text-gray-700">Orders</Link>
        )}
        <CartIcon />
      </div>
    </div>
  );
};

export default Navbar;
